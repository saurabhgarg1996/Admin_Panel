<?php
		...
		{
			iUpdateCats($category_id,$categoryname,$showinapp);	
			 if(!empty($_FILES['upload_category_image']["tmp_name"])){
                   ...
                   if(unlink($target_file)){
                   	header("location: ../remCategories.php?in=1");
                   	}
                   else {
                   	header("location: ../remCategories.php?in=3");
						...
						if (file_exists($target_file)) {
							header("location: ../remCategories.php?in=2");
						...
						if ($imageFileType != "jpg"){
							header("location: ../remCategories.php?in=4");
							$response="insert=failed";			
						}	
						if ($response == "insert=success"){
								if (move_uploaded_file($_FILES["upload_category_image"]["tmp_name"], $target_file)) {
									...
									$response = "insert=failed";
									header("location: ../remCategories.php?in=5");
			...
			$response = "insert=failed";
			header("location: ../remCategories.php?in=6");
         if($_SESSION['is_logged_in'] == false){
          	header("Location: ../logout.php");
         }		
		}                     
	...
	header("location: ../remCategories.php?in=7");
    if($_SESSION['is_logged_in'] == false){
       //header("Location: ../logout.php");
     }
	}
	...
	header("location: ../remCategories.php?in=8");
